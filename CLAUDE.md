...
**Commands.** Every command written for him to run is a full absolute path, every time, with no exception for the short ones and no `cd` in front of it. `X:\_CLAUDE\26_09_01_Job_Hunt\jobhunt\run.cmd stats`, never `run.cmd stats`. The one he actually lives in is `X:\_CLAUDE\26_09_01_Job_Hunt\jobhunt\app.cmd`, the desktop app, which holds the search console and the site sandbox in one window (ADR-0016). Everything in `run.cmd` is maintenance behind it. He runs these from wherever he happens to be, so a bare launcher name is a command that fails, and asking him to change directory first is asking him to do work the path already does. This has had to be said more than once, which is why it is written down here instead. Several commands go in one block, chained with `;`, never as a stack of separate blocks he has to copy one at a time. One paste, one run.
