...
The part I care about most is that the search protocol is a file, and the scraper runs it. Two hard gates come first, because two things are never worth a conversation. The role has to be actually remote, with no hybrid language, no state list that leaves out where I live, no payroll-default tells. The pay has to clear a floor, and an unlisted salary is tolerated only from a company I already rate. A posting that fails a gate isn't deleted, it's logged with the reason, so every week I can see what the gates threw away and whether they were right. Everything that survives gets a score out of a hundred, remote, comp, how many legs of the intersection the posting names, the title tier, the company tier, freshness, and a few points off for the phrases that mean underpaid. Sixty five and up with a title that fits is the apply pile, sorted by how much I want the company and then by score. Forty five and up, or anything with a flag on it, is the review pile, and that's the pile that matters, because a tool that silently drops the interesting edge cases is worse than no tool. Every score lands on the row with the ruleset version, so when I tune the protocol I can rescore months of history and see what would've changed. The digest also carries a footer that names any source that errored or returned nothing twice running, so when an endpoint changes shape I find out inside a week. I refresh the recorded fixture from the live payload, and the failing test shows me exactly which field moved.
