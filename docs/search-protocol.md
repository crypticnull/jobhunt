...
- `product`: figma prototyp, figma, design system, design systems, component library, interaction design, design tokens, motion system, motion systems, product animation, ui animation, interface animation, animation system, motion guidelines, micro-interaction, microinteraction, lottie, rive, bodymovin, protopie, origami, framer, smart animate, variable fonts, spline.design, spline 3d, motion tokens, motion principles, motion language, animation principles, design handoff. Added 2026-09-05 and trimmed the same day: the whole product-motion vocabulary went in first and the skeptics refused it, because prototyping, storybook and prototype are discipline words every product and frontend posting uses. The leg carries the tool names and the motion-system phrases, which name the work rather than the discipline, and deliberately does not satisfy the tier B leg gate on its own.
