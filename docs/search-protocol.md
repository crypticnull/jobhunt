...
- The location names hybrid, onsite or in-office, and does not also say remote. Checked against the location only. A location field is short and structured rather than prose, so the eighty-character lookahead that keeps `hybrid` from firing on body text is not needed there and its absence was a hole: "San Francisco Bay Area Hybrid" scored remote plus pacific and sat second in the apply pile. "Remote or Hybrid" still passes, because it says remote. Added 2026-09-06.
...
| Compensation | 20 | Midpoint at or over the top band: 20. Between the floor and the top band: 15. Unlisted at tier 1–3: 10. Unlisted, size > 200, no tier: 8. Flagged, between the flag floor and the pass floor: 5. The figures live in `data/local/scoring.local.json`. |
| Intersection asks | 20 | Legs are weighted: `product-motion` 10, `3d` and `generative` and `software` and `pipeline` 5 each, `product` 2, `motion` 0. Summed, capped at 20. Legs and their keyword lists are in `scoring.json`. A leg counts once no matter how many keywords hit. |
...
**Intersection legs** (weighted, summed, capped at 20)

Flat scoring was revised on 2026-09-06. Every leg had been worth five, so a posting saying `figma` scored the intersection exactly as hard as one saying you will own our motion system. In the 2026-W36 digest that put 39 of the 51 surfaced postings on the `product` leg while not one of the 51 had motion or animation in its title, which is the search quietly turning into a product design search. The leg was one list doing two jobs and is now two lists. `product-motion` is worth 10 because it names the work. `product` is worth 2 because it names the context. `motion` is worth nothing: it is the longform vocabulary, after effects and compositing and premiere, so it names the work the second principle rejects. A weight of nothing is not the same as not being a leg, and it still prints in the digest so a longform posting is visible as one.
...
- `product-motion`: 10 points. motion system, motion systems, product animation, ui animation, interface animation, animation system, motion guidelines, micro-interaction, microinteraction, lottie, rive, bodymovin, protopie, origami, framer, smart animate, spline.design, spline 3d, motion tokens, motion principles, motion language, animation principles. The work itself. Split out of `product` on 2026-09-06 and weighted double, because these are the words that say a posting has motion in it rather than that it is a design job.
- `product`: 2 points. figma prototyp, figma, design system, design systems, component library, interaction design, design tokens, variable fonts, design handoff. The context. Added 2026-09-05 and trimmed the same day, because prototyping, storybook and prototype are discipline words every product and frontend posting uses. Cut from 5 to 2 on 2026-09-06: a posting can say figma without any motion being in the job, and 39 of 51 surfaced postings in W36 did.

**Title tiers**
- Tier A (15): creative technologist, technical artist, design technologist (zero occurrences in 6,425 postings as of 2026-09-05, kept here because it costs nothing and would be a bullseye, removed from the discovery term lists where it only spent match budget), creative engineer, motion design engineer, creative developer, generative ai designer, generative ai artist, generative artist, ai creative lead, ai creative director, creative ai engineer, creative ai designer, creative ai artist, creative ai lead, creative ai director, pipeline td, pipeline technical director, motion technologist, technical director, motion systems, motion system designer, product motion, design systems engineer. Revised 2026-09-05: technical director and pipeline td need a generative, software, pipeline, product-motion or product leg, because Creature FX Technical Director at a VFX house tiered A and overrode every flag, and bare "creative ai" put Creative AI Product Manager in apply at 80.
- Tier B (10): motion designer, motion graphics designer, 3d motion designer, 3d artist, 3d generalist, art director, design engineer, motion lead, head of motion, animation director, **product designer, brand designer, visual designer**, and as of 2026-09-05 interaction designer, ux engineer, prototyper, design systems designer, motion design lead, motion director, product motion designer, when the body also hits the `generative`, `software`, `pipeline`, `product-motion` or `product` leg. The product-motion titles were untiered before, so Interaction Designer with a Figma and design-systems body scored 70 and could never reach apply.
...
- **Relevance floor**, added 2026-09-05 and tightened the same day. A posting is relevant when the title tiers, or the body hits a **craft** leg: `3d`, `motion`, `generative`, `product-motion` or `product`. `software` and `pipeline` are deliberately excluded, because they support a creative role rather than define one and every backend job in the corpus hits them. The first version accepted any leg, and the first real run put 894 postings in a pile led by LLM, CI/CD and GraphQL, which is a software job board rather than a shortlist. A posting with neither a tiered title nor a craft leg is logged whatever it scores. Remote 22 plus comp 20 plus a tier 1 company plus freshness is 57, over the review threshold, on a posting about nothing Matt does, which is how a Backend Engineer at a good company was reaching the review pile. With a cap of 40 a week that is how the pile fills with work he would never take. Comp and company prestige can carry a posting, but only after it is about something.
- **The craft leg has to be corroborated by the title**, added 2026-09-06. At an AI company the About Us block says generative and product in every posting, whatever the role, so the legs fired on an Economist, a Workday Engineer, a Commercial Counsel and a Revenue Accounting Manager exactly as they fired on a designer. Forty-five of the seventy-nine rows in the 2026-09-06 review pile were that shape. So the rescue now also needs a craft word in the title: creative, design, designer, motion, brand, visual, art, artist, animation, animator, technologist, interaction, ux, ui, prototyper, illustration, 3d, graphics or typography. A tiered title still passes on its own, and this governs the rescue only.
  The cost is stated rather than hidden. A Software Engineer posting that genuinely is the intersection reads identically to one carrying company boilerplate, so Staff Product Engineer at Runway and two frontend roles at product companies go with the rest. The two halves log under separate reasons, `no title fit and no intersection` and `intersection but the title names no craft`, so the digest shows how big the second pile is and the rule can be loosened on evidence.
